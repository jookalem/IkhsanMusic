...
async def start_stream_video(message, file, title, mystic):
    ...
    if await is_active_chat(message.chat.id):
        ...
        final_output = await message.reply_photo(
            ...
            caption=(
                f"📀 <b>Judul : </b> [Given Video Via Telegram]({link})\n\n🤴 <b>Requested by : </b>{message.from_user.mention} \n🚧 <b>Antrian Ke :</b> <b>#{position}!</b>"
            ),
        ...
        await mystic.delete()
        cap = f"📀 <b>Playing : </b>[Given Video Via Telegram]({link})\n🤴 **Requested by:** {message.from_user.mention}"
        final_output = await message.reply_photo(
            photo="Utils/Telegram.JPEG",
...
async def start_live_stream(
    ...
    cap = f"**Live Streaming**\n\n📀 <b>Playing : </b>[{title[:25]}](https://www.youtube.com/watch?v={videoid}) \n💡 [Get Additional Information](https://t.me/{BOT_USERNAME}?start=info_{videoid})\n🤴 **Requested by :** {CallbackQuery.from_user.mention}"
